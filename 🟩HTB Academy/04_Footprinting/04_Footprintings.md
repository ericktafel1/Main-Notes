...
---
## IMAP / POP3 #IMAP #POP3 #IMAPS #POP3S 
...
```shell
6165@htb[/htb]$ sudo nmap 10.129.14.128 -sV -p110,143,993,995 -sC
...
```shell
6165@htb[/htb]$ curl -k 'imaps://10.129.14.128' --user user:p4ssw0rd
...
```shell
6165@htb[/htb]$ curl -k 'imaps://10.129.14.128' --user cry0l1t3:1234 -v
...
```shell
6165@htb[/htb]$ openssl s_client -connect 10.129.14.128:pop3s
...
```shell
6165@htb[/htb]$ openssl s_client -connect 10.129.14.128:imaps
...
```shell
6165@htb[/htb]$ cat /etc/snmp/snmpd.conf | grep -v "#" | sed -r '/^\s*$/d'
...
```shell
6165@htb[/htb]$ snmpwalk -v2c -c public 10.129.14.128
...
```shell
6165@htb[/htb]$ sudo apt install onesixtyone
6165@htb[/htb]$ onesixtyone -c /opt/useful/SecLists/Discovery/SNMP/snmp.txt 10.129.14.128
...
```shell
6165@htb[/htb]$ sudo apt install braa
6165@htb[/htb]$ braa <community string>@<IP>:.1.3.6.*   # Syntax
6165@htb[/htb]$ braa public@10.129.14.128:.1.3.6.*
...
```shell
6165@htb[/htb]$ sudo apt install mysql-server -y
6165@htb[/htb]$ cat /etc/mysql/mysql.conf.d/mysqld.cnf | grep -v "#" | sed -r '/^\s*$/d'
...
```shell
6165@htb[/htb]$ sudo nmap 10.129.14.128 -sV -sC -p3306 --script mysql*
...
```shell
6165@htb[/htb]$ mysql -u root -h 10.129.14.132
...
```shell
6165@htb[/htb]$ mysql -u root -pP4SSw0rd -h 10.129.14.128
...
```shell
6165@htb[/htb]$ locate mssqlclient
...
```shell
6165@htb[/htb]$ sudo nmap --script ms-sql-info,ms-sql-empty-password,ms-sql-xp-cmdshell,ms-sql-config,ms-sql-ntlm-info,ms-sql-tables,ms-sql-hasdbaccess,ms-sql-dac,ms-sql-dump-hashes --script-args mssql.instance-port=1433,mssql.username=sa,mssql.password=,mssql.instance-name=MSSQLSERVER -sV -p 1433 10.129.201.248
...
```shell
6165@htb[/htb]$ python3 mssqlclient.py Administrator@10.129.201.248 -windows-auth
...
```shell
6165@htb[/htb]$ sudo nmap -p1521 -sV 10.129.204.235 --open
...
```shell
6165@htb[/htb]$ sudo nmap -p1521 -sV 10.129.204.235 --open --script oracle-sid-brute
...
```shell
6165@htb[/htb]$ ./odat.py all -s 10.129.204.235
...
```shell
6165@htb[/htb]$ sqlplus scott/tiger@10.129.204.235/XE
...
```shell
6165@htb[/htb]$ sudo sh -c "echo /usr/lib/oracle/12.2/client64/lib > /etc/ld.so.conf.d/oracle-instantclient.conf";sudo ldconfig
```
...
```shell
6165@htb[/htb]$ sqlplus scott/tiger@10.129.204.235/XE as sysdba
...
```shell
6165@htb[/htb]$ echo "Oracle File Upload Test" > testing.txt
6165@htb[/htb]$ ./odat.py utlfile -s 10.129.204.235 -d XE -U scott -P tiger --sysdba --putFile C:\\inetpub\\wwwroot testing.txt ./testing.txt
...
```shell
6165@htb[/htb]$ curl -X GET http://10.129.204.235/testing.txt
...
```shell
6165@htb[/htb]$ sudo nmap -sU --script ipmi-version -p 623 ilo.inlanfreight.local
...
```shell
6165@htb[/htb]$ cat /etc/ssh/sshd_config  | grep -v "#" | sed -r '/^\s*$/d'
...
```shell
6165@htb[/htb]$ git clone https://github.com/jtesta/ssh-audit.git && cd ssh-audit
6165@htb[/htb]$ ./ssh-audit.py 10.129.14.132
...
```shell
6165@htb[/htb]$ ssh -v cry0l1t3@10.129.14.132
...
```shell
6165@htb[/htb]$ ssh -v cry0l1t3@10.129.14.132 -o PreferredAuthentications=password
...
```shell
6165@htb[/htb]$ sudo nmap -sV -p 873 127.0.0.1
...
```shell
6165@htb[/htb]$ nc -nv 127.0.0.1 873
...
```shell
6165@htb[/htb]$ rsync -av --list-only rsync://127.0.0.1/dev
...
| ---------------- | ------------------ | -------- | ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rcp` #rcp       | `rshd`             | 514      | TCP                    | Copy a file or directory bidirectionally from the local system to the remote system (or vice versa) or from one remote system to another. It works like the `cp` command on Linux but provides `no warning to the user for overwriting existing files on a system`.        |
| `rsh` #rsh       | `rshd`             | 514      | TCP                    | Opens a shell on a remote machine without a login procedure. Relies upon the trusted entries in the `/etc/hosts.equiv` and `.rhosts` files for validation.                                                                                                                 |
| `rexec` #rexec   | `rexecd`           | 512      | TCP                    | Enables a user to run shell commands on a remote machine. Requires authentication through the use of a `username` and `password` through an unencrypted network socket. Authentication is overridden by the trusted entries in the `/etc/hosts.equiv` and `.rhosts` files. |
| `rlogin` #rlogin | `rlogind`          | 513      | TCP                    | Enables a user to log in to a remote host over the network. It works similarly to `telnet` but can only connect to Unix-like hosts. Authentication is overridden by the trusted entries in the `/etc/hosts.equiv` and `.rhosts` files.                                     |
...
```shell
6165@htb[/htb]$ cat /etc/hosts.equiv
...
```shell
6165@htb[/htb]$ sudo nmap -sV -p 512,513,514 10.0.17.2
...
#### Logging in Using Rlogin #rlogin

Linux Remote Management Protocols

```shell
6165@htb[/htb]$ rlogin 10.0.17.2 -l htb-student
...
#### Listing Authenticated Users Using Rusers #rusers
...
```shell
6165@htb[/htb]$ rusers -al 10.0.17.5
...
```shell
6165@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p3389 --script rdp*
...
```shell
6165@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p3389 --packet-trace --disable-arp-ping -n
...
```shell
6165@htb[/htb]$ git clone https://github.com/CiscoCXSecurity/rdp-sec-check.git && cd rdp-sec-check
6165@htb[/htb]$ ./rdp-sec-check.pl 10.129.201.248
...
```shell
6165@htb[/htb]$ xfreerdp /u:cry0l1t3 /p:"P455w0rd!" /v:10.129.201.248
...
```shell
6165@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p5985,5986 --disable-arp-ping -n
...
```shell
6165@htb[/htb]$ evil-winrm -i 10.129.201.248 -u Cry0l1t3 -p P455w0rD!
...
```shell
6165@htb[/htb]$ /usr/share/doc/python3-impacket/examples/wmiexec.py Cry0l1t3:"P455w0rD!"@10.129.201.248 "hostname"
