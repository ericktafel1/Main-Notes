...
```shell
6165@htb[/htb]$ cat id_rsa |base64 -w 0;echo
...
```shell
6165@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare
...
```shell
6165@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare -user test -password test
...
```shell
6165@htb[/htb]$ sudo pip3 install pyftpdlib
```
...
```shell
6165@htb[/htb]$ sudo python3 -m pyftpdlib --port 21
...
```shell
6165@htb[/htb]$ echo  | base64 -d > hosts
```

Windows File Transfer Methods

```shell
6165@htb[/htb]$ md5sum hosts 
...
```shell
6165@htb[/htb]$ pip3 install uploadserver
...
```shell
6165@htb[/htb]$ echo <base64> | base64 -d -w 0 > hosts
```
...
```shell
6165@htb[/htb]$ sudo pip3 install wsgidav cheroot
...
```shell
6165@htb[/htb]$ sudo wsgidav --host=0.0.0.0 --port=80 --root=/tmp --auth=anonymous 
...
```shell
6165@htb[/htb]$ sudo python3 -m pyftpdlib --port 21 --write
